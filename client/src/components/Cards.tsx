...
const Cards = (props) => {
  ...
  async function viewCourse() {
    console.log(id)
    // const data= await api.post(`/admin/courses/${id}`, null,{
    //   headers: {
  ...
  return (
    <>
      <div className="cards" style={{display:"flex", flexDirection:"column"}}>
        ...
        <button className='purchase-btn' onClick={viewCourse} style={{display:"flex", alignSelf:"center", justifyContent:"center",textAlign:"center", cursor:"pointer"}}> <span style={{display:"flex",alignSelf:"center"}}>View Course</span></button>
      </div>
    </>
  )
}
